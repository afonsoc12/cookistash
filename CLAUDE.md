...
1. **Prepare**: manually trigger `prepare-release.yml` (`workflow_dispatch`) - pick a `patch`/`minor`/`major` bump, or set the `version` input for an exact `X.Y.Z` override. `uv version --bump <type>` (or `uv version <override>`) is the source of truth: it bumps `pyproject.toml`/`uv.lock` first, then `release-flow/keep-a-changelog-action`'s `bump` command (same tool the release job already uses for `query`) is told that exact version to convert `CHANGELOG.md`'s `[Unreleased]` section into - the changelog follows uv's version, never computes its own. Commits straight to `main`, then creates and pushes the `vX.Y.Z` tag - all in one atomic run, no separate merge/tag step.
