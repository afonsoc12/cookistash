...
- **`release.yml`** (`workflow_dispatch`, pick a `patch`/`minor`/`major` bump) is the actual release pipeline, entirely self-contained in one workflow run:
  1. Runs `ci.yml` against the current `main` HEAD *before* touching anything - a version bump never gets committed for code that fails lint/type-check/tests. Bumping first and checking CI after would risk a version-bumped commit stuck on `main` with no release to show for it if CI then failed.
  2. Checks `git ls-remote` for a `vX.Y.Z` tag matching the version this bump would produce - if it already exists, fails loudly instead of silently bumping past a version whose release previously failed partway through and was never cleaned up.
  3. `uv version --bump <type>` bumps `pyproject.toml`/`uv.lock`; `release-flow/keep-a-changelog-action`'s `bump` command (pinned `@v3`) converts `CHANGELOG.md`'s `[Unreleased]` section using that same bump keyword - `@v3`'s `version` input only accepts a keyword (`major`/`minor`/`patch`/...), not an explicit version, despite what its `main`-branch (unreleased) docs claim.
  4. Commits and pushes straight to `main`, with `[skip ci]` in the commit message (see below), and captures the pushed commit's exact SHA as a job output.
  5. Calls `build.yml` (same reusable workflow `dev-release.yml` uses) pinned to that same SHA, with the real `:{version}`/`:latest` tags and no version suffix - `pyproject.toml` already has the bumped version committed by this point, so the wheel/sdist need no mutation either.
...
(An earlier version of this pipeline pushed the Docker image to a staging tag and only promoted it to `:latest`/`:{version}` after the release was verified, to close the gap where the image could exist before its release did. Dropped - the actual risk was a few minutes of an image existing slightly before the release page, which nobody would notice, and it wasn't worth an entire extra job, a second registry login, and a staging tag that lingers in GHCR forever.)
